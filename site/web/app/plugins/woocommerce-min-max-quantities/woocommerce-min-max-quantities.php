...
class WC_Min_Max_Quantities {
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			include_once( dirname( __FILE__ ) . '/includes/class-wc-min-max-quantities-admin.php' );
		}

		include_once( dirname( __FILE__ ) . '/includes/class-wc-min-max-quantities-addons.php' );
	...
	public function check_cart_items() {
		...
		foreach ( $category_quantities as $category => $quantity ) {
			...
			if ( $group_of_quantity > 0 && ( intval( $quantity ) % intval( $group_of_quantity ) > 0 ) ) {
				...
				if ( $product_names ) {
					$this->add_error( sprintf( __( 'Items in the <strong>%s</strong> category (<em>%s</em>) must be bought in groups of %d. Please add or remove the items to continue.', 'woocommerce-min-max-quantities' ), $term->name, implode( ', ', $product_names ), $group_of_quantity, $group_of_quantity - ( $quantity % $group_of_quantity ) ) );
					return;
				}
			}
		}
	...
	public function check_rules( $product, $quantity, $minimum_quantity, $maximum_quantity, $group_of_quantity ) {
		...
		if ( $minimum_quantity > 0 && $quantity < $minimum_quantity ) {
			...
			if ( $allow_combination && ( $product->is_type( 'variation' ) || $product->is_type( 'variable' ) ) ) {
				$this->add_error( sprintf( __( 'The minimum order quantity for %s is %s - please increase the quantity in your cart or add additional variation of this product.', 'woocommerce-min-max-quantities' ), $product->get_title(), $minimum_quantity ) );
			} else {
				$this->add_error( sprintf( __( 'The minimum order quantity for %s is %s - please increase the quantity in your cart.', 'woocommerce-min-max-quantities' ), $product->get_title(), $minimum_quantity ) );
			}

		} elseif ( $maximum_quantity > 0 && $quantity > $maximum_quantity ) {
			if ( $allow_combination && ( $product->is_type( 'variation' ) || $product->is_type( 'variable' ) ) ) {
				$this->add_error( sprintf( __( 'The maximum allowed quantity for %s is %s - please decrease the quantity in your cart or remove additional variation of this product.', 'woocommerce-min-max-quantities' ), $product->get_title(), $maximum_quantity ) );
			} else {
		...
		if ( $group_of_quantity > 0 && ( intval( $quantity ) % intval( $group_of_quantity ) > 0 ) ) {
			$this->add_error( sprintf( __( '%s must be bought in groups of %d. Please add or decrease items to continue.', 'woocommerce-min-max-quantities' ), $product->get_title(), $group_of_quantity, $group_of_quantity - ( $quantity % $group_of_quantity ) ) );
		}
	}
