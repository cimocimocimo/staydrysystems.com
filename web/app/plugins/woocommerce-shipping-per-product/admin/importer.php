...
if ( class_exists( 'WP_Importer' ) ) {
	...
	class WC_CSV_Per_Product_Shipping_Import extends WP_Importer {
		...
		function import( $file ) {
			...
			echo '<div class="updated settings-error below-h2"><p>
				'.sprintf( __( 'Import complete - imported <strong>%s</strong> shipping rates and skipped <strong>%s</strong>.', 'woocommerce' ), $this->imported, $this->skipped ).'
			</p></div>';
		...
		function header() {
			echo '<div class="wrap">';
			screen_icon();
			echo '<h2>' . __( 'Import Per-product Shipping Rates', 'woocommerce' ) . '</h2>';
		}
		...
		function greet() {
			...
			echo '<p>' . __( 'Hi there! Upload a CSV file containing per-product shipping rates to import the contents into your shop. Choose a .csv file to upload, then click "Upload file and import".', 'woocommerce' ).'</p>';

			echo '<p>' . __( 'Rates need to be defined with columns in a specific order (6 columns). Product ID, Country Code, State Code, Postcode, Cost, Item Cost', 'woocommerce' ) . '</p>';
